...
parser.add_argument('--output', action='store', type=str, required=True, help='directory with name')

parser.add_argument('--model', action='store', choices=('default', 'defaultnrom0', 'defaultnrom1', 
                                                        'log3ch', 'log5ch', 'original', 'circpad', 'circpadlog3ch', 'circpadlog5ch'),
...
if not os.path.exists(predFile):
    ...
    myDataset = MyDataset()
    basedir = os.environ['SAMPLEDIR'] if 'SAMPLEDIR' in  os.environ else "../data/hdf5_noPU_64x64/"
    ...
    model.load_state_dict(torch.load(args.input+'/weight_0.pkl'))#(args.input+'/weight_0.pkl', torch.device('cpu')))#(args.input+'/weight_0.pkl'))# we have no gpu
...
plt.legend()
plt.savefig(args.output+'_eval1.png')#
#plt.show()
...
plt.legend()
plt.savefig(args.output+'_eval2.png')#
#plt.show()
'''
plt.figure()
plt.plot(fpr, tpr, '.-', label='%s %.3f' % (((args.input).split('/'))[4:], auc))#(args.input, auc)) change label name
...
plt.ylim(0, 1.000)
plt.legend()
plt.savefig(args.output+'_eval3.png')#
#plt.show()

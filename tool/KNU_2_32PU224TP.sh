    ...
    do
      for i in `echo "adam"`
        do
          ...
          python train_labelByUser.py --model "$j" --lr 1e-3 --epoch 50 --batch 64 -o ~/work/result/20200825/run__model_"$j"__lr_1e-3__trackCh_pt__batch_64__optimizer_"$i"__pbs_"$k" --optimizer "$i" --device 2 --batchPerStep "$k" -c config.yaml
        done
    done
  done
